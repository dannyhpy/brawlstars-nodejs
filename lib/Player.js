...
class Player {
  constructor(data) {
    ...
    this.expPoints = data.expPoints
		this.calcExp = { expCurrent: getCurrentExp(data), expMax: getMaxExp(data), expNeeded: getNeededExp(data) }
...
function getCurrentExp(player) {
  let expMax = 40 + 10 * (player.expLevel - 1);
  let num = 0;
  let i;
    for (i = 0; i < (player.expLevel - 1); i++) {
      num = num + i;
    };
    let expCurrent = Math.abs((40 * (player.expLevel - 1)) + (10 * num) - player.expPoints);

    return expCurrent
}

function getMaxExp(player) {
  let expMax = 40 + 10 * (player.expLevel - 1);
  let num = 0;
  let i;
    for (i = 0; i < (player.expLevel - 1); i++) {
...
function getNeededExp(player) {
  ...
  let i;
    ...
    let expNeeded = expMax - expCurrent
