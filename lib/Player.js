...
class Player {
  constructor(data) {
    ...
    this.gadgetsCount = this.brawlers.map(value => value.gadgets).flat().length.toString();
    this.starpowersCount = this.brawlers.map(value => value.starPowers).flat().length.toString();
  }
